...
def prepare_coordinate_map():
    ...
    return unwarp.calculate_coordinate_map(map_point, grid_size, args.output_pixels_per_grid_unit)

unwarp_params = prepare_coordinate_map()
...
def process_all_images():
    with multiprocessing.Pool(processes=args.processes if args.processes > 0 else None) as p:
        return sum(p.map(process_image, args.input_files))


no_processed_images = process_all_images()
